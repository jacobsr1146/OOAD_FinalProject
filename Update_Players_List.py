...
def importFile(playersFile, goaliesFile):
    ...
    # NHLplayersstats_2019-20: ['Name', 'FTeam', 'Salary', 'Age', 'Pos', 'GP', 'G', 'A', 'P', 'PIM', '+/-', 'TOI', 'ES', 'PP', 'SH', 'ESG', 'PPG', 'SHG', 'GWG', 'OTG', 'ESA', 'PPA', 'SHA', 'GWA', 'OTA', 'ESP', 'PPP', 'SH
    # P', 'GWP', 'OTP', 'PPP%', 'G/60', 'A/60', 'P/60', 'ESG/60', 'ESA/60', 'ESP/60', 'PPG/60', 'PPA/60', 'PPP/60', 'G/GP', 'A/GP', 'P/GP', 'SHOTS', 'SH%', 'HITS', 'BS', 'FOW', 'FOL', 'FO%'

    # NHLgoaliestats_2019-20: ['Name', 'FTeam', 'Salary', 'Age', 'Pos', 'GP', 'W', 'L', 'W%', 'SO', 'GAA', 'SV%']
    ...
    for i in range(len(players)-1):         # take out PIM
        players[i].pop(9)
    for i in range(len(players)-1):         # take out TOI stats
        for j in range(5):
            players[i].pop(10)
    for i in range(len(players)-1):         # take out SHG
        players[i].pop(11)
